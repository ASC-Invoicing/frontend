import { useState, useEffect, useMemo, type FormEvent, type SetStateAction } from "react";
...
const EditInvoice = () => {
  ...
  const { data: invoiceData, isLoading: isInvoiceLoading } = useViewInvoiceQuery(
    { orgUID: orgUID!, invoiceUID: invoiceUID! },
    { skip: !orgUID || !invoiceUID }
  );
  ...
  const [draftInvoiceUID, setDraftInvoiceUID] = useState<string | null>(null);

  // --- Prefill form with existing invoice ---
  useEffect(() => {
    if (!invoiceData?.data) return;
    const inv = invoiceData.data;
    setCustomerTIN(inv.Business?.TIN || "");
    setInvoiceDate(inv.InvoiceDate);
    setDueDate(inv.DueDate);
    setNotes(inv.Notes || "");
    setDraftInvoiceUID(inv.UID);
  ...
  const handleAddFromProducts = async (productUID: string) => {
    ...
    if (!draftInvoiceUID || !orgUID) return;

    try {
      const res = await addLineItem({
        orgUID,
        InvoiceUID: draftInvoiceUID,
      ...
      setLineItems(prev =>
        prev.map(item => item.id === newItem.id ? { ...item, UID: res.data?.UID } : item)
      );
  ...
  const updateItem = async (id: number, field: keyof LineItem, value: string | number) => {
    setLineItems(prev =>
      prev.map(item => item.id === id ? { ...item, [field]: Number(value) || (value as any) } : item)
    );

    const item = lineItems.find(i => i.id === id);
    if (item?.UID && draftInvoiceUID && orgUID) {
      try {
  ...
  const handleRemoveItem = async (id: number) => {
    ...
    if (item?.UID && draftInvoiceUID && orgUID) {
      try {
        await deleteLineItem({ orgUID, invoiceUID: draftInvoiceUID, lineItemUID: item.UID }).unwrap();
  ...
  const handleSubmit = async (e: FormEvent, submitToFIRS = false) => {
    ...
    try {
      ...
      let res;
      if (!draftInvoiceUID || submitToFIRS) {
        res = await createInvoice(payload).unwrap();
        setDraftInvoiceUID(res.data.UID);
  ...
  const columns = [
    { title: "Description", dataIndex: "Description", render: (text: string, record: LineItem) =>
      <Input value={text} placeholder="Service or Product name"
  ...
  if (isInvoiceLoading) return <div className="min-h-screen flex justify-center">
<LoadingSpinner/>
  </div>;
      ...
      <div className="flex-1 flex justify-center pb-4">
        <div className="w-full max-w-5xl space-y-6">
          ...
          <div className="items-center">
            ...
            <div className="mt-6">
              <h2 className="text-2xl font-bold text-gray-900">Edit Invoice #{invoiceData?.data?.IRN}</h2>
              <p className="mt-1 text-sm text-gray-500">Edit invoice details and submit to FIRS if needed</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-12">
            ...
            <div className="bg-white rounded-xl border border-gray-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 p-6 lg:grid-cols-2 gap-6">
                <Input label="Customer TIN *" value={customerTIN} onChange={(e: { target: { value: SetStateAction<string>; }; }) => setCustomerTIN(e.target.value)} required />
                <Input label="Invoice Date *" type="date" value={invoiceDate} onChange={(e: { target: { value: SetStateAction<string>; }; }) => setInvoiceDate(e.target.value)} required />
                <Input label="Due Date" type="date" value={dueDate} onChange={(e: { target: { value: SetStateAction<string>; }; }) => setDueDate(e.target.value)} />
              </div>
            </div>
            ...
            <div className="space-y-6 bg-white rounded-xl border border-gray-200">
              <div className="flex justify-between items-baseline border-b border-gray-200 bg-[#F9FAFB] p-6">
                ...
                <div className="flex gap-3">
                  <Select
                    options={ProductOptions.map(opt => ({ value: opt.value, label: opt.label }))}
                    placeholder={isProductsLoading ? "Loading products..." : "Add from products"}
                  ...
                  <Button
                    ...
                    onClick={() => setLineItems(prev => [...prev, { id: Date.now(), Description: "", Quantity: 1, UnitPrice: 0, TaxRate: 7.5 }])}
                  >
                    Add Item
                  </Button>
                </div>
              ...
              <div className="p-6">
                <DataTable columns={columns} dataSource={lineItems} emptyText="No items added yet." bordered />
                <div className="flex justify-end mt-6 pt-4 border-t border-gray-200">
                  <div className="w-full max-w-xs space-y-2 text-sm">
                    <div className="flex justify-between font-medium text-gray-600"><span>Subtotal:</span><span>₦{subtotal.toFixed(2)}</span></div>
                    <div className="flex justify-between font-medium text-gray-600"><span>Tax (VAT):</span><span>₦{vat.toFixed(2)}</span></div>
                    <div className="flex justify-between text-lg font-bold text-[#00A859] pt-2 border-t border-gray-300"><span>Total:</span><span>₦{total.toFixed(2)}</span></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 space-y-4">
              <h3 className="text-xl font-semibold text-gray-800 border-b border-gray-200 bg-[#F9FAFB] p-6">Additional Information</h3>
              <CustomTextArea label="Notes" value={notes} onChange={(e: { target: { value: SetStateAction<string>; }; }) => setNotes(e.target.value)} className="px-6 pb-5" />
            </div>

            {/* Footer */}
            <div className="flex flex-wrap justify-between items-center mt-5 p-4 rounded-b-xl">
              <Button type="button" variant="ghost" className="bg-white border border-gray-200 text-gray-700" onClick={() => navigate("/invoices")}>
                <X className="w-4 h-4 mr-2" /> Cancel
              </Button>
              <div className="flex gap-3">
                <Button icon={<Send className="w-4 h-4" />} className="bg-[#9333EA] hover:bg-purple-700" onClick={e => handleSubmit(e, true)} type="button" disabled={isSubmitting} loading={isSubmitting} loadingText="Submitting">Submit to FIRS</Button>
                <Button icon={<FileText className="w-4 h-4" />} type="button" variant="solid" onClick={e => handleSubmit(e, false)} disabled={isSavingDraft} loading={isSavingDraft} loadingText="Saving">Save as Draft</Button>
              </div>
            </div>
